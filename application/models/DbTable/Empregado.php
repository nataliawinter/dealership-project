...
class Application_Model_DbTable_Empregado extends Zend_Db_Table_Abstract
{
	...
	public function addEmpregado($nomeCompleto, $email, $dataNasc, $cpf, $rg, $nroCarteiraTrab, 
		$telefone, $celular, $salario, $rua, $bairro, $numero, $cep, $cargo_idCargo, $tipoContrato, 
	...
	public function updateEmpregado($id, $nomeCompleto, $email, $dataNasc, $cpf, $rg, $nroCarteiraTrab,
		$telefone, $celular, $salario, $rua, $bairro, $numero, $cep, 
		$cargo_idCargo, $tipoContrato, $dataInicio, $cidade, $estado, $pais)
		{
			$data = [
				'NomeCompleto' => $nomeCompleto,
				'Email' => $email,
	...
	{
		$select = $this->select()->from('empregado', ['idEmpregado', 'NomeCompleto']);
		return $this->fetchAll($select);
	}
	
	public function verificaCargo($id)
	{
		$select = $this->select()->from('empregado', ['idEmpregado'])->where('Cargo_idCargo = ?', $id);
		return $this->fetchAll($select);
	}
		
	public function totalSalario()
	{
        $select = $this->select()->from('empregado', ['soma' => 'sum(Salario)']);
	...
	{
		$select = $this->select()->from('empregado', ['TotalComissao' => 'AcumulaComissao'])->where('idEmpregado = ?', $idEmpregado);
		$result = $this->fetchRow($select);
		...
		$data = ['AcumulaComissao' => $somaComissaoAtual];
